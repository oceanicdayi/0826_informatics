...
論文哪個章節，以及審閱過程中發現的問題。若想快速了解目前哪些圖已實際
插入論文、以及下一步該做什麼，請見
[`USAGE_AND_NEXT_STEPS.zh-TW.md`](USAGE_AND_NEXT_STEPS.zh-TW.md)。
...
| [`fig_case_study_hualien_m72.png`](fig_case_study_hualien_m72.png) | [`fig_case_study_hualien_m72.zh-TW.md`](fig_case_study_hualien_m72.zh-TW.md) | 花蓮 M7.2 單一事件詳圖——與 `08_submit_case_study_hualien_m72.md` 完全一致 |

## 本次審閱的重點發現

1. **兩張圖使用了未解釋的 n=44 事件群組。** `fig_alert_timeline.png` 與
   `fig_detection_categories.png` 都分析 44 個 I≥4 事件，但所有討論
   SSIF/eBEAR 比較的論文章節（07_、摘要）以及
   `fig_region_contingency.png` 用的都是 **50 個配對測試事件（39 個
   區域正例、11 個區域負例）**。`fig_detection_categories.png` 中
   「SSIF only」的數字（EW20 之後為 3）與摘要及 07_ 文件所述「eBEAR
   漏掉 2 個事件」對不上。詳見
...
4. 其餘檢查皆無問題：`fig_station_event_map.png` 的資料切分數量與
   ...
   `08_submit_case_study_hualien_m72.md` 逐項一致。
