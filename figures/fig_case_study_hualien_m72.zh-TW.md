...
此圖已用作 [`05_submit_Results.md`](../05_submit_Results.md) 文末「案例
研究：2024 年 M7.2 花蓮地震」子章節的配圖
（`![Case Study...](figures/fig_case_study_hualien_m72.png)`），
面板 (a) 表格中的每個數字都與該子章節的表格完全吻合。9.4 秒／M6.2 首次
警報與 14.0 秒／M6.8 修正值，也與同一子章節的「Comparison with eBEAR」
段落一致。
...
已正確放置——作為 [`05_submit_Results.md`](../05_submit_Results.md)
文末案例研究子章節的配圖。僅需修正空心菱形標記缺少圖例說明的問題。
